# QR-Based File Upload & Download System (Dockerized)

A simple, fast, and hassle-free way for printing-shop customers to share documents without WhatsApp or Email.
Customers scan a QR code → upload a file → shop staff download it from a secure dashboard.

**No user login. No app installs. No printer drivers or direct printing.**
Just scan → upload → download → print.

---

## 🚀 Features

### 👤 Customer Side
- Scan a QR code generated by the shop
- Upload PDF / Images / DOC / DOCX
- No need to save phone numbers or compose emails

### 🏬 Shop Side
- Generate a short-lived session (QR code)
- See all user-uploaded files in real time
- Download files directly to shop PC
- Files auto-expire and get deleted

### 🛠 Technical Features
- **Backend:** Node.js + Express + Multer
- **Frontend:** React (Vite)
- **File storage:** Docker Volume (uploads-data)
- Fully Dockerized using Dockerfile + docker-compose
- Automatic cleanup: expired sessions & files
- Configurable file limits, TTLs, and storage paths

---

## 🏗️ System Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                         CUSTOMER FLOW                           │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
                    ┌──────────────────┐
                    │   Scan QR Code   │
                    └──────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    FRONTEND (React + Vite)                      │
│                     Port: 5173                                  │
├─────────────────────────────────────────────────────────────────┤
│  • HomePage.jsx          → Generate QR & Session                │
│  • UploadPage.jsx        → Customer file upload UI              │
│  • ShopDashboard.jsx     → Shop file management UI              │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼ HTTP/REST API
┌─────────────────────────────────────────────────────────────────┐
│                BACKEND (Node.js + Express)                      │
│                     Port: 3000                                  │
├─────────────────────────────────────────────────────────────────┤
│  API Endpoints:                                                 │
│  • POST /api/sessions        → Create session & QR              │
│  • POST /api/upload/:token   → Upload file                      │
│  • GET  /api/jobs/:token     → List uploaded files              │
│  • GET  /api/files/:jobId    → Download file                    │
│  • GET  /api/health          → Health check                     │
├─────────────────────────────────────────────────────────────────┤
│  Middleware:                                                    │
│  • Multer (file upload)                                         │
│  • CORS                                                         │
│  • Express.json                                                 │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              FILE STORAGE (Docker Volume)                       │
│                   uploads-data                                  │
├─────────────────────────────────────────────────────────────────┤
│  • Persistent storage across container restarts                 │
│  • Auto-cleanup based on TTL                                    │
│  • Organized by session tokens                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                      DATA FLOW                                  │
├─────────────────────────────────────────────────────────────────┤
│  1. Shop → Create Session → Backend generates token + QR       │
│  2. Customer → Scan QR → Opens upload page with token          │
│  3. Customer → Upload file → Stored in Docker volume            │
│  4. Shop → View dashboard → See all uploaded files              │
│  5. Shop → Download file → Retrieve from storage                │
│  6. Auto-cleanup → Expired sessions & files deleted             │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📦 Project Structure

```
file-sharing-site/
├─ backend/
│  ├─ Dockerfile
│  ├─ package.json
│  ├─ server.js
│  ├─ .env
│  └─ uploads/ (mounted as docker volume)
├─ frontend/
│  ├─ Dockerfile
│  ├─ package.json
│  ├─ vite.config.js
│  ├─ vercel.json
│  ├─ index.html
│  └─ src/
│     ├─ main.jsx
│     ├─ App.jsx
│     ├─ App.css
│     └─ pages/
│        ├─ HomePage.jsx
│        ├─ HomePage.css
│        ├─ UploadPage.jsx
│        ├─ UploadPage.css
│        ├─ ShopDashboard.jsx
│        └─ ShopDashboard.css
├─ docker-compose.yml
├─ test-api.html
├─ .gitignore
└─ README.md
```

---

## 🐳 Run with Docker

### 1️⃣ Clone the project
```bash
git clone https://github.com/yourusername/file-sharing-site.git
cd file-sharing-site
```

### 2️⃣ Build & start all services
```bash
docker-compose up --build
```

### 3️⃣ Visit the services

| Service | URL |
|---------|-----|
| Frontend (React) | http://localhost:5173 |
| Backend API | http://localhost:3000 |

Backend health check:
```bash
GET http://localhost:3000/api/health
```

### 4️⃣ Stop containers
```bash
docker-compose down
```

Want to also remove uploaded files?
```bash
docker-compose down -v
```

---

## 🔧 How it Works

### 1. Shop Creates a Session

**Send:**
```bash
POST http://localhost:3000/api/sessions
Content-Type: application/json

{ "shopId": "shop-1" }
```

**Response:**
```json
{
  "token": "a1b2c3d4e5",
  "uploadUrl": "http://localhost:5173/upload/a1b2c3d4e5",
  "qrDataUrl": "data:image/png;base64,...",
  "expiresAt": 1713456000000
}
```

The shop displays the QR code to the customer.

### 2. Customer Uploads a File

**Open:**
```
http://localhost:5173/upload/<token>
```

**Upload via cURL:**
```bash
curl -F "file=@document.pdf" http://localhost:3000/api/upload/<token>
```

### 3. Shop Views All Files

**Open:**
```
http://localhost:5173/shop/<token>
```

Files appear automatically every 5 seconds.

**API:**
```bash
GET http://localhost:3000/api/jobs/<token>
```

### 4. Download File
```bash
GET http://localhost:3000/api/files/<jobId>   (Browser download)
```

File will be saved from the backend's volume-mapped directory.

---

## 🗂 Docker Volume for Uploads

Uploaded files are stored inside:
```
/app/uploads
```

mapped to the named Docker volume:
```
uploads-data
```

**Inspect:**
```bash
docker volume inspect uploads-data
```

**Extract files to host:**
```bash
docker run --rm -v uploads-data:/data -v $(pwd):/backup alpine cp -r /data /backup
```

---

## ⚙️ Environment Variables

**Backend:** `.env`
```env
PORT=3000
PUBLIC_URL=http://localhost:5173
UPLOAD_DIR=./uploads
SESSION_TTL_MIN=30
FILE_TTL_HOURS=24
MAX_FILE_SIZE_BYTES=52428800
```

**Frontend:** No .env needed for local development. For deployment, configure via vercel.json or hosting platform.

---

## 🔒 Security Notes

- Session tokens auto-expire
- Files auto-deleted after TTL
- File types restricted (PDF, images, DOC, DOCX)
- Max file size configurable (default 50MB)
- QR URL is unguessable (UUID-based tokens)
- Only shop UI can download files (dashboard URL not meant to be public)
- **Use HTTPS in production**

---

## 📌 Limitations (MVP)

- Uses in-memory session storage → resets if backend restarts
- No authentication for shop dashboard (can be added)
- Not meant for multi-shop scaling yet
- No direct printing (by design — file download only)

---

## 🚀 Future Improvements

- Add PostgreSQL or Redis for sessions/jobs
- Add Socket.io for real-time uploads
- Add ClamAV for virus scanning
- Add shop login (JWT-based)
- Deploy to Docker Swarm or Kubernetes
- Support S3 bucket storage instead of local volume

---

## ✔️ Summary

This project provides a clean, fast, Dockerized solution for print shops to receive customer documents without WhatsApp, Gmail, or pen drives — all with QR codes and a clean UI.
